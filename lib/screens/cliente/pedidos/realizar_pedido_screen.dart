...
class _RealizarPedidoScreenState extends State<RealizarPedidoScreen> {
  ...
  // Genera un ID único de ticket: PAR-YYYYMMDD-XXXXX
  ...
  Future<void> _confirmarPedido() async {
    ...
    try {
      ...
      final ticket = _generarTicketId();
      await orders.createFromCart(cart.items);
      await cart.clear();
      setState(() { _ticketId = ticket; _success = true; _procesando = false; });
    } catch (_) {
      setState(() {
      ...
      child: Column(mainAxisSize: MainAxisSize.min,
        ...
        const Row(children: [
          Icon(Icons.payment_outlined,
              color: AppColors.clientePrimary, size: 22),
          SizedBox(width: 10),
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start,
            ...
            const Row(children: [
              Icon(Icons.account_balance_outlined,
                  size: 16, color: AppColors.cajeroNavbar),
              SizedBox(width: 6),
              Text('Realiza tu transferencia a:',
                  style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700,
                      color: AppColors.cajeroNavbar)),
            ]),
            const SizedBox(height: 12),
...
class _DatosBancariosRow extends StatelessWidget {
  ...
  final bool copiable;
  const _DatosBancariosRow(this.label, this.value,
      {this.copiable = false});
  ...
  Widget build(BuildContext context) {
    return Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      Container(
        width: 80, height: 80,
        decoration: const BoxDecoration(
            color: Color(0xFFD4F1E4), shape: BoxShape.circle),
        child: const Icon(Icons.check_rounded,
            color: AppColors.clientePrimary, size: 46),
      ),
      const SizedBox(height: 16),
      const Text('¡Pedido registrado!',
          style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900)),
      const SizedBox(height: 6),
      const Text('Tu pedido está pendiente de verificación de pago.',
        ...
        child: Column(children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(10),
              border: Border.all(
                  color: AppColors.clientePrimary, width: 2),
            ),
